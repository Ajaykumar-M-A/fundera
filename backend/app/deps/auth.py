...
def require_approved_user(user: User = Depends(get_current_user)) -> User:
    if user.status != "APPROVED":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Waiting for Admin Approval")
    return user


def require_trading_user(user: User = Depends(get_current_user)) -> User:
    if user.status != "APPROVED":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Waiting for Admin Approval")
